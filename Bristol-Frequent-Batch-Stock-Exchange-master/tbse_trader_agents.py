...
class Trader_PRZI(Trader):
    ...
    def __init__(self, ttype, tid, balance, params, time):
        ...
        self.strat_wait_time = 10    # how many secs do we give any one strat before switching?
    ...
    def getorder(self,time,p_eq ,q_eq, demand_curve,supply_curve,countdown,lob):
        ...
        # shvr_price tells us what price a SHVR would quote in these circs
         def shvr_price(otype, limit, lob):
            # calculate the price at which a rational trader would quote
            if otype == 'Bid':
                if lob['bids']['n'] > 0:
        ...
        # calculate cumulative distribution function (CDF) look-up table (LUT)
         ...
         if len(self.orders) < 1:
            ...
            for entry in lut['cdf_lut']:
                if u < entry['cum_prob']:
                    quote_price = entry['price']
                    break

            order = Order(self.tid, self.orders[coid].otype, quote_price, self.orders[coid].qty, time, self.orders[coid].coid,
                          self.orders[coid].toid)
                          #  self.orders[coid].otype is self.job which is mentioned for several times above
